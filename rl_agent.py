...
class RL_Agent:
    ...
    def pg_grad(self, states, actions, rewards, baselines=None):
        ...
        # It is assumed states are passed with [None, ...] shape, actions and rewards have [None, 1] shape
        # For this reason I reshape actions before passing them to one_hot (otherwise shape would be [None, 1, 2])
        action_mask = tf.one_hot(actions.reshape(-1), depth=self.n_actions, on_value=1.0, off_value=0.0, axis=-1)
        picked_log_prob_actions = tf.reduce_sum(action_mask * self.log_prob_layer, axis=1)
        if baselines is None:
            weighted_log_prob_actions = picked_log_prob_actions * rewards.reshape(-1)
        else:
            weighted_log_prob_actions = picked_log_prob_actions * (rewards - baselines).reshape(-1)
    ...
    def trpo_grad(self, states, actions, rewards):
        ...
        weighted_prob_ratio = masked_prob_ratio * rewards.reshape(-1)
    ...
    def compare_prob_ratios(self, states, actions, rewards, d_var):
        ...
        action_mask = tf.one_hot(actions.reshape(-1), depth=self.n_actions, on_value=1.0, off_value=0.0, axis=-1, dtype=tf.float32)
